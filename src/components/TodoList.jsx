    ...
    <div className="max-w-xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-0.5">
          {active.length === 0 && todos.length > 0
            ? 'All done! 🎉'
            : `${active.length} task${active.length !== 1 ? 's' : ''} to do`}
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4 mb-5 flex items-center gap-3">
        ...
        <button
          onClick={add}
          disabled={!input.trim()}
          className="bg-sage-500 text-white p-2 rounded-xl hover:bg-sage-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
          <Plus size={17} />
        </button>
      </div>
          ...
          {active.map((todo, i) => (
            <div
              ...
              className={`flex items-center gap-3 px-5 py-3.5 group ${
                i < active.length - 1 ? 'border-b border-cream-100' : ''
              }`}
            >
              <button
                onClick={() => toggle(todo.id)}
                className="w-5 h-5 rounded-full border-2 border-sage-300 hover:border-sage-500 flex-shrink-0 flex items-center justify-center hover:bg-sage-50 transition-all"
              />
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden opacity-60">
            {done.map((todo, i) => (
              <div
                key={todo.id}
                className={`flex items-center gap-3 px-5 py-3.5 group ${
                  i < done.length - 1 ? 'border-b border-cream-100' : ''
                }`}
              >
                <button
                  onClick={() => toggle(todo.id)}
                  className="w-5 h-5 rounded-full bg-sage-400 flex-shrink-0 flex items-center justify-center hover:bg-sage-300 transition-colors"
                >
